...
本书不会讨论ES5新增特性相关的模式，因为在本书截稿时并没有任何浏览器实现了ES5（译注：截止译稿校对时，Chrome/Firefox/IE9+（部分）已实现ES5，具体兼容情况可参考<http://kangax.github.com/es5-compat-table/>），但本书的示例代码有以下特点，以鼓励开发者向新标准转变：
